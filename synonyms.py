...
stopwords = open('resources/stopwords_small_origin.txt').read().split('\n')
...
def generateVariantKeywords(untokenize_text):
    ...
    for i in range(0, len(tokens)):
        ...
        sy_keyword = [word.replace('_', ' ')]
        pos = tokens[i][1]
        if pos in map_pos:
            pos = map_pos[pos]
            if word in map_synonym[pos]:
                sy_keyword.extend([syno.replace('_', ' ') for syno in map_synonym[pos][word]])
        keywords.append(sy_keyword)
